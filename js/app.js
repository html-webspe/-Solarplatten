(() => {
    ...
    function initSliders() {
        ...
        if (document.querySelector(".works__slider")) new core(".works__slider", {
            modules: [ Navigation, Manipulation, EffectCoverflow ],
            loop: true,
            speed: 1e3,
            autoplay: {
                delay: 3e3
    ...
    da.init();
    const calculateBtn = document.querySelector(".calculate-js");
    const quizClose = document.querySelector(".form-quiz__close");
    const stepsForm = document.querySelector(".quiz");
    calculateBtn.addEventListener("click", (function(e) {
        stepsForm.classList.add("quiz-open");
    }));
    quizClose.addEventListener("click", (function(e) {
        stepsForm.classList.remove("quiz-open");
    }));
    function stepForm() {
        const stepsForm = document.querySelector(".form-quiz");
        const steps = document.querySelectorAll(".form-quiz__step");
        const stepsProgress = document.querySelector(".form-quiz__progresbar");
        const stepsBtnPrev = document.querySelector(".form-quiz__btn-prev");
        const stepsBtnNext = document.querySelector(".form-quiz__btn-next");
        if (stepsForm) {
            let formStepIndex = 0;
            stepsBtnNext.addEventListener("click", (function(e) {
                formStepIndex++;
                updateFormSteps();
                const span = document.createElement("span");
                span.style.width = 100 / (steps.length - 1) + "%";
                stepsProgress.appendChild(span);
            }));
            stepsBtnPrev.addEventListener("click", (function(e) {
                formStepIndex--;
                updateFormSteps();
                stepsProgress.lastChild.remove();
            }));
            function updateFormSteps() {
                steps.forEach((step => {
                    step.classList.remove("step-quiz-current");
                }));
                steps[formStepIndex].classList.add("step-quiz-current");
                stepsBtnNext.style.display = formStepIndex === steps.length - 1 ? "none" : "block";
                stepsProgress.style.display = formStepIndex === steps.length - 1 ? "none" : "flex";
                stepsBtnPrev.style.display = 0 === formStepIndex || formStepIndex === steps.length - 1 ? "none" : "block";
            }
            updateFormSteps();
        }
    }
    stepForm();
    const formStep = document.getElementById("form-quiz");
    if (formStep) {
        formStep.addEventListener("submit", formSend);
        async function formSend(e) {
            e.preventDefault();
            const formAction = formStep.getAttribute("action") ? formStep.getAttribute("action").trim() : "#";
            let formData = new FormData(formStep);
            formStep.classList.add("sending");
            ...
            if (response.ok) {
                ...
                formStep.classList.remove("_sending");
                formStep.reset();
            } else {
                formStep.classList.remove("sending");
